...
def process_job(job_data):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            ...
            tts_audio_path = os.path.join(tmpdir, "generated_speech.mp3")
            ...
            transcribed_text = transcribe(audio_path, source_lang)
            print(f"[TRANSCRIBE] {transcribed_text[:200]}...")

            print(f"[INFO] Translating for job {job_id}")
            translated_text = translate(transcribed_text, target_lang)
            print(f"[TRANSLATE] {translated_text[:200]}...")

            print(f"[INFO] Generating speech for job {job_id}")
            generate_speech(translated_text, tts_audio_path, target_lang)

            print(f"[INFO] Merging audio for job {job_id}")
            merge(video_path, tts_audio_path, output_video_path)
